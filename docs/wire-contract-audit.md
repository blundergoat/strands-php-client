...
The result: the PHP client consumes custom Python/FastAPI wrapper contracts, not raw sdk-python `TypedDict` shapes.

## Audited Projects

Evidence paths belong to other repositories. Each path starts with its repository name and is otherwise relative to that repository's root.

| Project | Contract usage | Evidence |
| --- | --- | --- |
| `the-summit-chatroom` | Uses standard `/invoke` and `/stream` endpoints with `message`, `session_id`, and `context`. Emits canonical SSE event types such as `text`, `thinking`, `tool_use`, `tool_result`, `complete`, and `error`. | `the-summit-chatroom/strands_agents/api/server.py` (search: `InvokeRequest`, `build_complete_event`, `map_sdk_event`) |
| `ambient-scribe` | Uses custom endpoints through `postJson()` and wrapper-specific Pydantic schemas. Project docs explicitly treat PHP/Python request models as coupled contracts. | `ambient-scribe/docs/workflow.md` (search: `Pydantic request model breaks`), `ambient-scribe/src/Controller/ScribeController.php` (search: `postJson`) |
| `halaxy-agents-lab` | Uses custom file metadata endpoints and stream endpoints. Shared schemas emit snake_case `AgentUsage`; helpers normalize sdk-python usage counters before PHP receives them. | `halaxy-agents-lab/strands_agents/api/schemas/shared.py` (search: `AgentUsage`), `halaxy-agents-lab/strands_agents/api/helpers.py` (search: `_extract_usage`) |
| `healthkit` | Uses several custom endpoints for chat, intent, metadata, summarise, and suggested actions. Shared wrapper helpers normalize usage and stream `complete` payloads before PHP consumption. | `healthkit/strands_agents/api/helpers.py` (search: `_extract_usage`), `healthkit/src/App/ExternalProvider/AI/StrandsAgents/FileSummariserStreamOrchestrator.php` (search: `streamSse`) |
...
- Wrapper helper logic is duplicated across services. `healthkit` and `halaxy-agents-lab` both normalize sdk-python usage into snake_case, so contract
  fixtures must cover that boundary until the logic is extracted.
...
- No shared Python package enforces the contract yet. Wrapper repositories should test against `tests/Fixtures/wire-contract/` until one exists.
...
`tests/Unit/Contract/ConsumerCompatibilityTest.php` provides executable coverage through consumer-shaped fixtures in
`tests/Fixtures/wire-contract/`.

## Recommendation

Keep `strands-php-client` aligned to `docs/wire-contract.md`.

Before adding PHP support for an sdk-python feature, update the wrapper contract and fixtures. Add PHP support only after active wrappers pass them.
