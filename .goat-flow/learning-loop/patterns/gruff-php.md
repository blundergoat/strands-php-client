...
**Evidence from this repo:** `naming.parameter-type-name` 466 → 1; `naming.abbreviation-allowlist` 188 → 0; zero BC breaks; tests + PHPStan Level 10 green. The one remaining is `$now = new \DateTimeImmutable()` in `src/Auth/SigV4Auth.php`, which the rule cannot silence for local vars (see `.goat-flow/learning-loop/footguns/gruff-php.md`).
