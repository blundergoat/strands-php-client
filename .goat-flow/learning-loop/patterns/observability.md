...
**Evidence:** `src/Http/ResponseObserver.php` (search: `afterStreamSse`) defines terminal hooks. `src/Http/Middleware/OtelTracingMiddleware.php` (search: `implements RequestMiddleware, ResponseObserver`) consumes both surfaces. `src/Http/ResponseObserverNotifier.php` (search: `normaliseResponseObservers`) auto-detects observer middleware and deduplicates explicit registrations.
