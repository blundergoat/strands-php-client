...
PHP 8.2+ library that acts as a thin client for a remote Strands agent (typically a Python FastAPI service). It is intentionally **stateless and loop-free**: every method either marshals one HTTP POST to the agent or streams an SSE response — the agentic reasoning loop runs on the server side. The wire boundary is the Strands HTTP Wire Contract v1 — see `docs/wire-contract.md` and `.goat-flow/decisions/ADR-001-strands-http-wire-contract.md`.

## System Overview

Five cooperating components, separated so a host application can swap any of them without touching the others:
...
- **Response/Streaming DTOs** (`src/Response/`, `src/Streaming/`) — `AgentResponse`, `Usage`, `StopReason`, `GuardrailTrace`, `InterruptDetail`, `StreamEvent`, `StreamEventType`, `StreamResult`. All readonly, all hydrated via static `fromArray()` factories with defensive type checks.
...
8. `RequestMiddleware[*]::afterResponse()` observes completion; exceptions in `afterResponse` are swallowed by design (observers must not break the call).
9. Interrupts are surfaced as `InterruptDetail[]`; the caller can resume with `AgentInput::interruptResponse(...)`.
...
- Both integrations share `StrandsClientFactory` (`src/Integration/StrandsClientFactory.php`) so transport detection, auth-driver resolution, and middleware wiring stay identical across frameworks.

## Deployment / Operations

This is a library — there is no runtime to deploy. Quality gates run via `composer preflight` locally and in `.github/workflows/ci.yml` on every push:
...
Releases tag from `main`. Branch alias `dev-main` → `1.4.x-dev` in `composer.json`. Packagist publishes on tag.
