...
run_full() {
  ...
  # Working-tree and history destruction beyond push/commit/reset --hard/clean -f. These were
  # allowed until the PR #6 review measured them, so each rule keeps a case here: a guard fix that
  # is not asserted is lost the next time the policy is rebuilt.
  expect_block writes "git checkout -- ." "git checkout pathspec"
  expect_block writes "git checkout main" "git checkout branch"
  expect_block writes "git switch main" "git switch"
  expect_block writes "git restore --staged --worktree ." "git restore"
  expect_block writes "git reset --keep HEAD~5" "git reset keep"
  expect_block writes "git rebase --onto main dev" "git rebase"
  expect_block writes "git merge --no-ff feature" "git merge"
  expect_block writes "git cherry-pick abc123" "git cherry-pick"
  expect_block writes "git revert HEAD" "git revert"
  expect_block writes "git am patch.mbox" "git am"
  expect_block writes "git apply --index patch.diff" "git apply"
  expect_block writes "git pull --rebase origin main" "git pull"
  expect_block writes "git sparse-checkout set src" "git sparse-checkout"
  expect_block writes "git filter-branch --force --index-filter x HEAD" "git filter-branch"
  expect_block writes "git rm -rf src" "git rm"
  expect_block writes "git prune" "git prune"
  expect_block writes "git mv -f a b" "git mv force"
  expect_block writes "git stash" "git stash"
  expect_block writes "git stash drop" "git stash drop"
  expect_block writes "git bisect start" "git bisect start"
  expect_block writes "git submodule update --force --checkout" "git submodule update"
  expect_block writes "git submodule foreach rm -rf ." "git submodule foreach"
  expect_block writes "git branch -D dev" "git branch delete"
  expect_block writes "git branch -M main" "git branch force-move"
  expect_block writes "git tag -d v1.5.0" "git tag delete"
  expect_block writes "git reflog expire --expire=now --all" "git reflog expire"
  expect_block writes "git gc --prune=now --aggressive" "git gc prune"
  expect_block writes "git repack -a -d" "git repack delete"
  expect_block writes "git update-ref -d refs/heads/dev" "git update-ref delete"
  expect_block writes "git replace --delete abc" "git replace delete"
  expect_block writes "git notes prune" "git notes prune"
  expect_block writes "git worktree remove --force ../wt" "git worktree remove"
  expect_block writes "git -C /tmp checkout -- ." "git -C checkout"
  expect_block writes "timeout 5 git stash" "timeout wrapped git stash"
  expect_block writes "echo x | git reset --keep HEAD" "piped git reset keep"

  # The same rules must leave read-only evidence and the documented dry runs alone, or a developer
  # loses the commands they use to inspect the repository before deciding what to do.
  expect_allow writes "git branch -vv" "git branch verbose listing"
  expect_allow writes "git branch --merged main" "git branch merged listing"
  expect_allow writes "git tag -l 'v*'" "git tag listing"
  expect_allow writes "git tag -a v1.5.0 -m 'Release v1.5.0'" "git tag annotated create"
  expect_allow writes "git stash list" "git stash list"
  expect_allow writes "git stash show" "git stash show"
  expect_allow writes "git reflog" "git reflog listing"
  expect_allow writes "git gc" "git gc without prune"
  expect_allow writes "git repack -a" "git repack without delete"
  expect_allow writes "git bisect log" "git bisect log"
  expect_allow writes "git submodule status" "git submodule status"
  expect_allow writes "git worktree list" "git worktree list"
  expect_allow writes "git notes show HEAD" "git notes show"
  expect_allow writes "git rm --dry-run src" "git rm dry run"
  expect_allow writes "git prune -n" "git prune dry run"
  expect_allow writes "git clean -n" "git clean dry run"
  expect_allow writes "git apply --check patch.diff" "git apply check only"
  expect_allow writes "git mv a b" "git mv without force"
  expect_allow writes "git merge-base main dev" "git merge-base is not merge"
  expect_allow writes "git fetch origin" "git fetch"
