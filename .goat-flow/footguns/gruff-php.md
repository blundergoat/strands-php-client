...
**Defensive workflow:** allowlist BC-sensitive parameter names in `.gruff-php.yaml` (`naming.parameter-type-name.options.ignoredParameterNames`) rather than renaming. See `.goat-flow/patterns/gruff-php.md` for the workflow.
...
**Concrete case:** the bulk rename script for `naming.abbreviation-allowlist` would have corrupted `$author`-style names in tests had it used plain replace_all. See `.goat-flow/patterns/gruff-php.md` for the regex pattern.
